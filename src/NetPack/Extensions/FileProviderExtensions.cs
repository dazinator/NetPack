...
namespace NetPack
{
    ...
    public static class FileProviderExtensions
    {
        ...
        public static long GetActualPosition(this StreamReader reader)
        {
            ...
            var charBuffert = type.GetMember("charBuffer");

            var charBufferField = type.GetField("charBuffer", flags);
            char[] charBuffer = (char[])charBufferField.GetValue(reader);

            var charPosField = type.GetField("charPos", flags);
            int charPos = (int)charPosField.GetValue(reader);

            var charLenField = type.GetField("charLen", flags);
            int charLen = (int)charLenField.GetValue(reader);

            var byteBufferField = type.GetField("byteBuffer", flags);
            byte[] byteBuffer = (byte[])byteBufferField.GetValue(reader);

            var byteLenField = type.GetField("byteLen", flags);
            int byteLen = (int)byteLenField.GetValue(reader);

            // The current buffer of decoded characters
            // var charBufferMember = 
            // char[] charBuffer = (char[])type.GetField("charBuffer");

            // , flags, null, reader, null);
